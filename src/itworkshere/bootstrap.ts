...
export function formatLocalSetupGuide(plan: LocalSetupPlan) {
  const requiredVariables = plan.environmentVariables.filter(
    (variable) => variable.required && !variable.hasTemplate
  );
  const lines = [
    "## DevGuard Bootstrap Plan",
    "",
  ...
  if (requiredVariables.length > 0) {
    ...
    for (const variable of requiredVariables) {
      lines.push(`- Provide a value for ${variable.name} in .env or your local environment.`);
    }
  }
...
function detectInstallCommands(readmeCommands: string[], packageManager: string | null) {
  ...
  if (packageManager === "npm") {
    return [{ command: "npm install", purpose: "install" as const, source: "package-lock.json" }];
  }

  if (packageManager === "pnpm") {
    return [{ command: "pnpm install", purpose: "install" as const, source: "pnpm-lock.yaml" }];
  }

  if (packageManager === "yarn") {
    return [{ command: "yarn install", purpose: "install" as const, source: "yarn.lock" }];
  }
...
function detectStartCommands(readmeCommands: string[], packageJson: PackageJson | null) {
  ...
  if (packageJson?.scripts?.dev) {
    return [{ command: "npm run dev", purpose: "start" as const, source: "package.json scripts.dev" }];
  }

  if (packageJson?.scripts?.start) {
    return [{ command: "npm start", purpose: "start" as const, source: "package.json scripts.start" }];
  }
...
function detectVerificationCommands(readmeCommands: string[], packageJson: PackageJson | null) {
  ...
  if (packageJson?.scripts?.test) {
    return [{ command: "npm test", purpose: "verify" as const, source: "package.json scripts.test" }];
  }
...
function buildBootstrapBlockers(input: {
  ...
  if (input.environmentMap.codeVariableReferences.length > 0 && input.environmentMap.envExampleKeys.length === 0) {
...
function calculateConfidence(input: { readmePath: string | null; blockers: string[]; assumptions: string[] }) {
