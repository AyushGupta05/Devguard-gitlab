...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    input[type=text], textarea { width: 100%; padding: 10px 12px; background: #252837; border: 1px solid #3a3f5c; border-radius: 6px; color: #e2e8f0; font-size: 14px; outline: none; }
    textarea { min-height: 220px; font-family: monospace; font-size: 13px; resize: vertical; }
    input[type=text]:focus, textarea:focus { border-color: #5b6bde; }
...
<main>
  ...
  <div class="card" id="import-card" style="display:none">
    ...
    <p style="font-size:13px;color:#7c85a2;margin-bottom:12px">Paste the full agent response or the JSON payload from Duo Chat.</p>
    <label for="agent-output">Agent output</label>
    <textarea id="agent-output" placeholder="Paste the entire agent response here."></textarea>
    <div style="margin-top:10px;display:flex;gap:10px">
      <button class="btn-primary" onclick="importSession()">Import and Create Session</button>
...
<script>
  ...
  async function importSession() {
    ...
    try {
      ...
      btn.disabled = false;
      btn.textContent = 'Import and Create Session';
    }
  }
  ...
  function toggleApproval(cmdId) {
    if (pendingApprovals.has(cmdId)) pendingApprovals.delete(cmdId);
    else pendingApprovals.add(cmdId);
  ...
  function renderSession(session) {
    ...
    const blockerHtml = session.blockers.map(b => '<div class="blocker">Warning ' + b + '</div>').join('');
    const commandsHtml = session.commandRequests.map(cmd => {
      ...
      const outputHtml = cmd.stdout || cmd.stderr
        ...
        : '';
      return '<div class="cmd-row"><div class="cmd-meta"><div class="cmd-label">' + cmd.label + '</div><div class="cmd-code">$ ' + cmd.command + '</div><div class="cmd-purpose" style="color:' + purposeColor(cmd.purpose) + '">' + cmd.purpose + ' | ' + cmd.source + '</div>' + outputHtml + '</div><div style="display:flex;gap:8px;align-items:center;flex-shrink:0">' + (canApprove ? '<button id="approve-btn-' + cmd.id + '" class="status-badge ' + badgeClass + '" onclick="toggleApproval(\\'' + cmd.id + '\\')">' + badgeText + '</button>' : '<span class="status-badge ' + badgeClass + '">' + cmd.status + '</span>') + '</div></div>';
    }).join('');
    container.innerHTML = '<div class="card"><div class="session-header"><div class="session-title">' + session.source.owner + '/' + session.source.name + '</div><div class="session-id">' + session.runId + '</div></div>' + guidanceHtml + blockerHtml + '<h2 style="margin-top:' + ((guidanceHtml || blockerHtml) ? '16px' : '0') + '">Commands</h2>' + (commandsHtml || '<div class="empty">No commands detected.</div>') + (hasBlocked ? '<div class="section-actions"><button class="btn-success" id="execute-btn" onclick="executeSession()" ' + (pendingApprovals.size === 0 ? 'disabled' : '') + '>Execute Approved Commands</button><button class="btn-ghost" onclick="approveAll()">Approve All</button></div>' : allDone ? '<div style="margin-top:16px;color:#86efac;font-size:13px">All commands completed.</div>' : '') + '</div>';
  }
  function approveAll() {
    const sessionContainer = document.getElementById('session-container');
    const buttons = sessionContainer.querySelectorAll('[id^="approve-btn-"]');
    buttons.forEach(btn => {
...
function sessionFromAgentPayload(payload: AgentSessionPayload): RemoteBootstrapSession {
  ...
  if (payload.requiredSecrets?.length) blockers.push(`Secrets needed before running: ${payload.requiredSecrets.join(", ")}`);
  if (payload.requiredConfig?.length) blockers.push(`Config vars needed: ${payload.requiredConfig.join(", ")}`);
  ...
  return remoteBootstrapSessionSchema.parse({
    ...
    cloneRequired: commandRequests.some((command) => command.purpose === "clone"),
    localSetupPlan: null,
    commandRequests,
    blockers,
    guidance: [
...
async function router(req: IncomingMessage, res: ServerResponse) {
  ...
  if (method === "POST" && url.pathname === "/api/sessions/import-text") {
    try {
      ...
      if (!payload) {
        sendError(res, 422, "No devguard:bootstrap-session payload found. Make sure you copied the full agent response.");
        return;
      }
  ...
  if (method === "POST" && url.pathname === "/api/sessions") {
    try {
      ...
      const session = buildRemoteBootstrapSession({ repoUrl: body.repoUrl, workspaceRoot: body.workspaceRoot });
      sessions.set(session.runId, session);
      send(res, 201, session);
    } catch (error) {
      sendError(res, 400, error instanceof Error ? error.message : String(error));
  ...
  if (method === "GET" && url.pathname === "/api/sessions") {
    send(res, 200, [...sessions.values()].map((session) => ({
      runId: session.runId,
      repo: `${session.source.owner}/${session.source.name}`,
      provider: session.source.provider,
      total: session.commandRequests.length,
      completed: session.commandRequests.filter((command) => command.status === "completed").length,
      failed: session.commandRequests.filter((command) => command.status === "failed").length,
      blocked: session.commandRequests.filter((command) => command.status === "blocked" || command.status === "pending").length
    })));
    return;
  }
...
server.listen(PORT, () => {
  console.log("");
  console.log("DevGuard Bootstrap Server");
  console.log(`  UI:  http://localhost:${PORT}`);
  console.log(`  API: http://localhost:${PORT}/api/sessions`);
  console.log("");
  console.log("Paste a repo URL in the browser to start a session.");
  console.log("");
});
