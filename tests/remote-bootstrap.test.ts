...
describe("remote bootstrap session", () => {
  ...
  it("keeps missing env values as blockers instead of executable commands", async () => {
    const sourceRoot = mkdtempSync(join(tmpdir(), "itwh-source-env-"));
    const workspaceRoot = mkdtempSync(join(tmpdir(), "itwh-clones-env-"));
    ...
    try {
      ...
      writeFileSync(join(sourceRoot, "index.js"), "console.log(process.env.API_KEY);\n");
      ...
      expect(executed.blockers.some((blocker) => blocker.includes("API_KEY"))).toBe(true);
      expect(executed.commandRequests.some((request) => request.command.includes("Create .env entries for"))).toBe(false);
    } finally {
      rmSync(sourceRoot, { recursive: true, force: true });
      rmSync(workspaceRoot, { recursive: true, force: true });
    }
